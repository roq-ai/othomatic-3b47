...
import { getCaseNotesById } from 'apiSdk/case-notes';
import { CaseNotesInterface } from 'interfaces/case-notes';

function CaseNotesViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<CaseNotesInterface>(
    () => (id ? `/case-notes/${id}` : null),
    () =>
      getCaseNotesById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('case_notes', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/case-notes/edit/${id}`} passHref legacyBehavior>
                    <Button
