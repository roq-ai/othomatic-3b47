...
import { getCaseBookings } from 'apiSdk/case-bookings';
import { CaseBookingInterface } from 'interfaces/case-booking';
import { getCaseNotes } from 'apiSdk/case-notes';
...
import { CaseContactsListPage as ShowCaseList } from 'pages/case-contacts';

function DashboardPage() {
  const { session } = useSession();

  const caseBookingStat = useSWR<PaginatedInterface<CaseBookingInterface>>(
    () => `/case-bookings`,
    () => getCaseBookings({ limit: 1, offset: 0 }),
  );

  const caseNotesStat = useSWR<PaginatedInterface<CaseNotesInterface>>(
    () => `/case-notes`,
    () => getCaseNotes({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Case Bookings',
      result: caseBookingStat?.data?.totalCount,
      isLoading: caseBookingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Case Notes',
      result: caseNotesStat?.data?.totalCount,
      isLoading: caseNotesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Renamedcases',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Case Contacts
                </Text>
                <NextLink href={`case-contacts`} passHref legacyBehavior>
                  <Button
