...
import { getBailiffs } from 'apiSdk/bailiffs';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'bailiff', 'accountant', 'clerk'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BailiffInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/bailiffs` : null),
    () => getBailiffs({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Bailiffs',
      path: isTenantUser ? `/bailiffs/view/${data?.[0]?.id}` : '/bailiffs',
      entity: 'bailiff',
