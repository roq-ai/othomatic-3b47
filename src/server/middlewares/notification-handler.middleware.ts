...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'payment_made.create': {
    roles: ['owner', 'bailiff', 'accountant', 'clerk'],
    key: 'payment',
    tenantPath: [],
    userPath: [],
  },
  'complaint_filed.create': {
    roles: ['owner', 'bailiff', 'accountant', 'clerk'],
    key: 'complaint',
    tenantPath: [],
    userPath: [],
  },
  'maintenance_completed.update': {
    roles: ['owner', 'bailiff', 'accountant', 'clerk'],
    key: 'maintenance',
    tenantPath: [],
    userPath: [],
...
const tenantRoles: string[] = ['owner', 'bailiff', 'accountant', 'clerk'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.bailiff.findFirst({
